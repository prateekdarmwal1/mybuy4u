...
class DesignerCloth extends AppModel
{

    var $name = "DesignerCloth";

    var $useTable = "designer_cloth";
    ...
    var $hasOne = array(
        'DesignerClothImage' => array(
            'className' => 'DesignerClothImage',
            'conditions' => '',
            'order' => '',
            'limit' => '',
    ...
    function get_designer_clothes_by_shop_id($id)
    {
        try {
            $this->unbindModel(array("belongsTo" => array("Shop")));
            $results = $this->find("all", array("conditions" => array("DesignerCloth.SHOP_ID" => $id),
                                          "fields" => array("DesignerCloth.ID", "NAME", "DESCRIPTION","COST_UNIT",
                                                                 "COST", "SIZE", "DesignerClothImage.IMAGE_PATH",
                                                                 "DesignerClothImage.IMAGE_PATH_BIG", "DesignerClothImage.ID")));
            return $results;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
    ...
    function get_designercloths_detail() {
		try {
			/*$this->unbindModel(array("belongsTo" => array("Shop")));*/
			$fields = array("DesignerCloth.ID", "DesignerCloth.NAME", "DesignerCloth.DESCRIPTION");
			$result = $this->find("all", array("conditions" => array("DesignerCloth.ID"), "fields"=>$fields));
			return $result;
		} catch (Exception $exception) {
			echo $exception->getMessage();
		}
    ...
    function get_designercloth_detail($id)
    {
        try {
            $this->unbindModel(array("belongsTo" => array("Shop")));
            $result = $this->find("first", array("conditions" => array("DesignerCloth.ID" => $id)));
            return $result;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
    ...
    function set_designerCloth_data($id)
    {
        try {
            $fields = array("Shop.ID", "NAME", "SIZE", "COST", "DESCRIPTION");
            $results = $this->find("first", array("conditions" => array('DesignerCloth.ID' => $id), "fields" => $fields));
            return $results;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
