 ...
 */
class GiftShops extends AppModel
{

    var $name = "GiftShops";

    var $useTable = "gift_shops";
    ...
    var $hasOne = array(
        'GiftShopsImage' => array(
            'className' => 'GiftShopsImage',
            'conditions' => '',
            'order' => 'GiftShops.ID',
            'limit' => '',
            'foreignKey' => 'GIFT_SHOPS_ID',
            'dependent' => true,
            'exclusive' => true,
            'finderQuery' => '',
    ...
    function get_gift_shops_by_shop_id($id)
    {
        try {
            $this->unbindModel(array("belongsTo" => array("Shop")));
            $results = $this->find("all", array("conditions" => array("Gift_Shops.SHOP_ID" => $id),
                "fields" => array("Gift_Shops.ID", "NAME", "DESCRIPTION", "COST_UNIT",
                    "COST", "SIZE", "GiftShopImage.IMAGE_PATH",
                    "GiftShopsImage.IMAGE_PATH_BIG", "GiftShopsImage.ID")));
            return $results;
    ...
    function get_giftshops_by_shop_id($id)
    {
        try {
            $this->unbindModel(array("belongsTo" => array("Shop")));
            $fields = array("giftShops.ID", "giftShops.NAME", "giftShops.DESCRIPTION", "TYPE", "COST",
                "COST_UNIT", "giftShops.IMAGE_PATH","giftShops.IMAGE_PATH_BIG", "giftShops.ID");
            $results = $this->find("all", array("conditions" => array("giftShops.SHOP_ID" => $id), "fields"=>$fields));
            return $results;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
    ...
    function get_giftshops_detail($id) {
        try {
            $this->unbindModel(array("belongsTo" => array("Shop")));
            $fields = array("giftShops.ID", "giftShops.NAME", "giftShops.DESCRIPTION",  "TYPE", "COST",
                "COST_UNIT","BRAND_NAME","SIZE");
            $result = $this->find("first", array("conditions" => array("giftShops.ID" => $id), "fields"=>$fields));
            return $result;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
...
/*
    function get_big_image_path($id)
    {
        try {
            $this->unbindModel(array("belongsTo" => array("Shop")));
            $result = $this->GiftShopImage->find("first", array("conditions" => array("BOUTIQUE_ID" => $id),
