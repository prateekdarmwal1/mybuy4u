...
class Kitchen extends AppModel
{
    ...
    function get_kitchen_by_shop_id($id)
    {
        try {
            $this->unbindModel(array("belongsTo" => array("Shop")));
            $results = $this->find("all", array("conditions" => array("Kitchen.SHOP_ID" => $id),
                "fields" => array("Kitchen.ID", "NAME", "DESCRIPTION","COST_UNIT",
                    "COST","KitchenImage.IMAGE_PATH",
                    "KitchenImage.IMAGE_PATH_BIG", "KitchenImage.ID")));
            return $results;
    ...
    function get_kitchen_detail($id)
    {
        try {
            $this->unbindModel(array("belongsTo" => array("Shop")));
            $result = $this->find("first", array("conditions" => array("Kitchen.ID" => $id)));
            return $result;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
    ...
    function get_kitchens_detail() {
        try {
            /*$this->unbindModel(array("belongsTo" => array("Shop")));*/
            $fields = array("Kitchen.ID", "Kitchen.NAME", "Kitchen.DESCRIPTION");
            $result = $this->find("all", array("conditions" => array("Kitchen.ID"), "fields"=>$fields));
            return $result;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
    ...
    function set_kitchen_data($id)
    {
        try {
            $fields = array("Shop.ID", "NAME", "COST","DESCRIPTION");
            $results = $this->find("first", array("conditions" => array('Kitchen.ID' => $id), "fields" => $fields));
            return $results;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
