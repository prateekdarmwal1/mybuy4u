...
class Product extends AppModel
{
    ...
    var $belongsTo = array(
        /*'Owner' => array(
            'className' => 'Owner',
            'foreignKey' => 'OWNER_ID',
            'fields' => array("NAME", "CONTACT_NUMBER", "MOBILE_NUMBER"),
        )
    ,
        'Address' => array(
            'className' => 'Address',
    ...
    function  get_detail_by_product_id($id)
    {
    ...
    function get_products_by_offset($offset)
    {
     ...
     function  get_detail_of_product_id($id)
    {
        try {
            $result = $this->find('all', array('conditions' => array('Product.PRODUCT_BRAND_ID' => $id)));
            return $result;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
