...
class ProductImage extends AppModel
{
    ...
    {
        try {
            $result = $this->find("first", array("conditions" => array('ProductImage.ID' => $id)));
            return $result;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
    ...
    {
        try {
            $results = $this->find('all', array('conditions' => array('ProductImage.PRODUCT_ID' => $id),
    ...
    {
        $this->query("UPDATE product_image SET IMAGE_PATH='" . $image_path . "'," . "IMAGE_PATH_BIG='" . $image_path_big . "'  WHERE ID=".$image_id);
    }
}
