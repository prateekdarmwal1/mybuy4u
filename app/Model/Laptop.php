...
class Laptop extends AppModel
{
    ...
    function get_laptop_by_shop_id($id)
    {
        try {
            $this->unbindModel(array("belongsTo" => array("Shop")));
            $results = $this->find("all", array("conditions" => array("Laptop.SHOP_ID" => $id),
                "fields" => array("Laptop.ID", "NAME", "DESCRIPTION","COST_UNIT",
                    "COST","LaptopImage.IMAGE_PATH",
                    "LaptopImage.IMAGE_PATH_BIG", "LaptopImage.ID")));
            return $results;
    ...
    function get_laptop_detail($id)
    {
        try {
            $this->unbindModel(array("belongsTo" => array("Shop")));
            $result = $this->find("first", array("conditions" => array("Laptop.ID" => $id)));
            return $result;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
    ...
    function get_laptops_detail() {
        try {
            /*$this->unbindModel(array("belongsTo" => array("Shop")));*/
            $fields = array("Laptop.ID", "Laptop.NAME", "Laptop.DESCRIPTION");
            $result = $this->find("all", array("conditions" => array("Laptop.ID"), "fields"=>$fields));
            return $result;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
    ...
    function set_laptop_data($id)
    {
        try {
            $fields = array("Shop.ID", "NAME", "COST","DESCRIPTION");
            $results = $this->find("first", array("conditions" => array('Laptop.ID' => $id), "fields" => $fields));
            return $results;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
