...
class Land extends AppModel
{
    ...
    function  get_land_detail($id)
    {
        try {
            $result = $this->find('all', array('conditions' => array('Land.REAL_ESTATE_ID' => $id)));
            return $result;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
    ...
    function  get_lands_detail($id)
    {
        try {
            $result = $this->find('first', array('conditions' => array('Land.REAL_ESTATE_ID' => $id)));
            return $result;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
    ...
    function  get_detail_by_land_id($id)
    {
        try {
    ...
    {
        try {
            $fields = array("Land.ID", "OWNER_ID", "ADDRESS", "COST", "COST_UNIT", "LAND_TYPE", "SIZE",'UPDATED_DATE');
        ...
        try {
            ...
            if (!empty($results)) {
                foreach ($results as $result) {
                    $temp_result["ID"] = $result["Land"]["ID"];
                    $temp_result["NAME"] = $result["Land"]["NAME"];
                    $temp_result["DESCRIPTION"] = $result["Land"]["DESCRIPTION"];
                    $temp_result["COST"] = $result["Land"]["COST"];
