...
class Interior extends AppModel
{

    var $name = "Interior";

    var $useTable = "interior";
    ...
    function get_interiors_by_shop_id($id)
    {
        try {
            $this->unbindModel(array("belongsTo" => array("Shop")));
            $results = $this->find("all", array("conditions" => array("Interior.SHOP_ID" => $id),
                                          "fields" => array("Interior.ID", "NAME", "DESCRIPTION","COST_UNIT",
                                                                 "COST","InteriorImage.IMAGE_PATH",
                                                                 "InteriorImage.IMAGE_PATH_BIG", "InteriorImage.ID")));
            return $results;
    ...
    function get_interior_detail($id)
    {
        try {
            $this->unbindModel(array("belongsTo" => array("Shop")));
            $result = $this->find("first", array("conditions" => array("Interior.ID" => $id)));
            return $result;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
    ...
    function get_interiors_detail() {
		try {
			/*$this->unbindModel(array("belongsTo" => array("Shop")));*/
			$fields = array("Interior.ID", "Interior.NAME", "Interior.DESCRIPTION");
			$result = $this->find("all", array("conditions" => array("Interior.ID"), "fields"=>$fields));
			return $result;
		} catch (Exception $exception) {
			echo $exception->getMessage();
		}
    ...
    function check_dimension($data)
    {
     ...
     function set_interior_data($id)
    {
        try {
            $fields = array("Shop.ID", "NAME", "COST","DESCRIPTION");
            $results = $this->find("first", array("conditions" => array('Interior.ID' => $id), "fields" => $fields));
            return $results;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
