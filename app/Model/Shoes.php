...
class Shoes extends AppModel
{
    ...
    function get_shoes_by_shop_id($id)
    {
        try {
            $this->unbindModel(array("belongsTo" => array("Shop")));
            $results = $this->find("all", array("conditions" => array("Shoes.SHOP_ID" => $id),
                "fields" => array("Shoes.ID", "NAME", "DESCRIPTION","COST_UNIT",
                    "COST","ShoesImage.IMAGE_PATH",
                    "ShoesImage.IMAGE_PATH_BIG", "ShoesImage.ID")));
            return $results;
    ...
    function get_shoes_detail($id)
    {
        try {
            $this->unbindModel(array("belongsTo" => array("Shop")));
            $result = $this->find("first", array("conditions" => array("Shoes.ID" => $id)));
            return $result;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
    ...
    function get_shoess_detail() {
        try {
            /*$this->unbindModel(array("belongsTo" => array("Shop")));*/
            $fields = array("Shoes.ID", "Shoes.NAME", "Shoes.DESCRIPTION");
            $result = $this->find("all", array("conditions" => array("Shoes.ID"), "fields"=>$fields));
            return $result;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
    ...
    function set_shoes_data($id)
    {
        try {
            $fields = array("Shop.ID", "NAME", "COST","DESCRIPTION");
            $results = $this->find("first", array("conditions" => array('Shoes.ID' => $id), "fields" => $fields));
            return $results;
        } catch (Exception $exception) {
            echo $exception->getMessage();
        }
