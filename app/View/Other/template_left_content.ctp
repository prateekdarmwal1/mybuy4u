...
<div class="yk_template_slider_content" style="padding:8px;">
    <div align="center">
        <h3><?php echo isset($results["Other"]["NAME"])?$results["Other"]["NAME"]:''; ?></h3>
        <h4>Reg. No. - <?php echo isset($results["Other"]["REGISTRATION_NO"])?$results["Other"]["REGISTRATION_NO"]:''; ?></h4>
    </div>
    <div class="yk_template_slider_content_left" style="min-height: 285px;">
        <b>Owner</b><br>
        <?php echo isset($results["Other"]["OWNER"])?$results["Other"]["OWNER"]:'';?><br>
        <br><b>Description</b><br>
        <?php echo isset($results["Other"]["DESCRIPTION"])?$results["Other"]["DESCRIPTION"]:'';?><br>
    ...
    <div class="yk_template_slider_content_right">
        ...
        <div class="yk_image1">
            <?php
                ...
                //debug($results);exit;
                //debug($results);exit;
                if (!empty($results["OtherImage"]["IMAGE_PATH"])) {
                    $img_path = $results["OtherImage"]["IMAGE_PATH"];
                    $img_path = explode("/", $img_path);
                    $temp = implode("/", array_slice($img_path, 0, 6));
                    $img_path_new = str_replace("/", "-", $temp);
                    $img_path_product = "http://" . $_SERVER["SERVER_NAME"] . $results["OtherImage"]["IMAGE_PATH"];
