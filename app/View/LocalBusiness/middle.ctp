<?php
    //debug($results);die;
    if (!empty($results['Shop'])) {
        $this->assign('head_description', 'mybuy4u local business - shop - ' . $results["Shop"]["NAME"] . ' - ' . $results["Shop"]["DESCRIPTION"]);
        $this->assign('head_keywords', 'mybuy4u local business - shop - ' . $results["Shop"]["NAME"] . ' - ' . $results["Shop"]["DESCRIPTION"]);
    }
?>
    ...
    <div class="yk_template_left_content_tag1_inner1">
        <div class="template_left_content_tag1_inner1_left">
            <div class="yk_template_left_content_user">
                <?php
                if (!empty($results['Owner']["OwnerImage"]["IMAGE_PATH"])) { ?>
...
<!--                --><?php //debug($new_path_new);exit;
                    echo $this->Html->image($new_path_new, array('width' => 104, 'height' => 107));
//                    debug($results['Owner']["OwnerImage"]["IMAGE_PATH"]);exit;?>
                <?php } else { ?>
    ...
    <?php
    //if ($this->Session->read("OWNER_NAME_ID_ROLE")) {
    ...
    ?>
        ...
        <div class="yk_template_left_content_tag1_inner2">
            <div class="yk_template_my_business">
                <?php if ( ( $this->Session->read('OWNER_NAME_ID_ROLE') ) AND ($name_id[1] == $results['Shop']['USER_ID'])  ){
                ?>
                <div class="yk_left_link"> <span class="yk_product_text ">
                <a href="#"
                   class="yk_right_content_link yk_font_12 rowUp">
                    <?php echo $this->Html->link('My Business',["controller"=>"users",'action'=>"edit_profile"], array('ng-href'=>'#/users/edit_profile/')
                        );?></a></span>
                </div>
                <?php
                }
                ...
                <a href="#"
                   class="yk_right_content_link yk_font_12 rowUp">
                    ...
                    if ( (( $this->Session->read('OWNER_NAME_ID_ROLE')) AND ($name_id[1] == $results['Shop']['USER_ID'] ))){
                        echo $this->Html->link('Edit Shop',["controller"=>"local_business",'action'=>"update_business_detail",$shop_id,$model_name], array('ng-href'=>'#/local_business/update_business_detail/' . $shop_id . '/' . $model_name.'/'));
                    }else{
                        echo "<a onmousedown='jqueryDialog(event)'>Edit Shop</a>";
                    }
                    ?></a></span>
                </div>
                ...
                <a href="#"
                   class="yk_right_content_link rowUp yk_font_12">
                    <?php
                    if ( ( ( $this->Session->read('OWNER_NAME_ID_ROLE') ) AND ($name_id[1] == $results['Shop']['USER_ID'])  ) ) {
                        echo $this->Html->link('Adv Setting',["controller"=>"local_business",'action'=>"adv_setting",$shop_id,$model_name]);
                    }else{
                ...
                <a href="#"
                   class="yk_right_content_link rowUp yk_font_12">
                    <?php
                    if ( ($this->Session->read('OWNER_NAME_ID_ROLE')) AND ($name_id[1] == $results['Shop']['USER_ID'])) {
                        echo $this->Html->link('Add Product',["controller"=>"local_business",'action'=>"add_product",$results["Shop"]["SHOP_CATEGORY"],$shop_id,$model_name],array('ng-app'=>'#/local_business/add_product/'. $results["Shop"]["SHOP_CATEGORY"] . "/" . $shop_id . '/' . $model_name));
                    }else{
                ...
                <a href="#"
                   class="yk_right_content_link rowUp yk_font_12">
                    <?php
                    if ( ($this->Session->read('OWNER_NAME_ID_ROLE')) AND ($name_id[1] == $results['Shop']['USER_ID'])) {
                        echo $this->Html->link('Add Page',["controller"=>"local_business",'action'=>"add_page",$shop_id,$model_name], array('ng-app'=>'#/local_business/add_page/' . $shop_id . '/' . $model_name.'/'));
                    }else{
                ...
                <a href="#"
                   class="yk_right_content_link rowUp yk_font_12">
                    <?php
                    if ( ($this->Session->read('OWNER_NAME_ID_ROLE')) AND ($name_id[1] == $results['Shop']['USER_ID'])) {
                        echo $this->Html->link('Adv Sale',["controller"=>"local_business",'action'=>"sales_detail",$results["Shop"]["SHOP_CATEGORY"],$shop_id,$model_name], array('ng-app'=>'#/local_business/sales_detail/' . $results["Shop"]["SHOP_CATEGORY"] . '/' . $shop_id . '/' . $model_name. '/'));
                    }else{
                        echo "<a onmousedown='jqueryDialog(event)'>Adv Sale</a>";
                    }
                    ?>
                </a> </span></div>
            </div>
...
<!--        <div class="yk_template_left_content_tag2_inner1 yk_template_left_content_left_div yk_width_tag180">-->
            <div class="yk_template_slider_content_left_description">
                <div class="yk_template_publish  ">
                    <span class="yk_template_publish"><br>
                        <?php if ($this->Session->read("OWNER_NAME_ID_ROLE")) {
                            $name_id = $this->Session->read("OWNER_NAME_ID_ROLE");
                            $name_id = explode(",", $name_id);
                            //debug($name_id[1]);exit;
                            if ($name_id[1] == $results['Shop']['USER_ID']) {
                                echo $this->Html->link('Publish advertisement in home page',["controller"=>"local_business","action"=>"ad_banner",$shop_id,$model_name]);

                            } else {
                                echo $this->Html->link('Publish advertisement in home page', "javascript:void(0)", array("onclick" => "message()"));
                            }
                        } else {
                            //debug($shop_id);die;
                            echo $this->Html->link('Publish advertisement in home page',["controller"=>"local_business","action"=>"ad_banner",$shop_id,$model_name]);
        ...
        <div class="yk_template_left_content_tag2_inner1 yk_width_tag180">
            <div class="yk_template_slider_content_left_description ">
                <div class="yk_template_publish yk_top_25">
                    <?php echo $this->Html->link('SMS',"javascript:void(0)");?>
<!--                    --><?php //echo $this->Html->link('SMS',["controller"=>"local_business","action"=>"#sms",$shop_id,$model_name], array('ng-href'=>'#/local_business/sms/'.$results["Shop"]["SHOP_CATEGORY"]."/". $shop_id.'/'.$model_name.'/'));?>
        ...
        <a href=''>
            <div class="yk_template_left_content_tag2_inner1 yk_width_tag180 ">
                <div class="yk_tag2_sales ">
                    <span class="yk_template_publish1 yk_font_12_bold1">
                        <?php if ($this->Session->read("OWNER_NAME_ID_ROLE")) {
                        $name_id = $this->Session->read("OWNER_NAME_ID_ROLE");
                        $name_id = explode(",", $name_id);
                        if ($name_id[1] == $results['Shop']['USER_ID']) {
                            ?>
                            <?php echo $this->Html->link('SALE Promotion', array("controller" => "homes", "action" => "home_sale_detail/")); ?></span>
                            <?php
        ...
        <div class=" yk_template_left_content_tag2_inner1  yk_template_left_content_right_div yk_width_tag180 yk_border_none">
            <div class="yk_tag2_sales yk_template_publish yk_font_12_bold">
                ...
                if ($name_id[1] == $results['Shop']['USER_ID']) {
                    ?>
                    <?php echo $this->Html->link('Promotional news Letter','#/local_business/news_letter/'. $shop_id . '/' . $model_name, array('ng-href'=>'#/local_business/news_letter/'.$shop_id.'/'.$model_name)); ?>
<!--                    --><?php //echo $this->Html->link('Promotional news Letter', array("controller" => "local_business", "action" => "news_letter/" /*. $result["Shop"]["SHOP_CATEGORY"] . "/" */ . $shop_id . '/' . $model_name)); ?>
                    ...
                    echo $this->Html->link('Promotional news Letter','#/local_business/news_letter/'. $shop_id . '/' . $model_name, array('ng-href'=>'#/local_business/news_letter/'.$shop_id.'/'.$model_name.'/',"javascript:void(0)",array("onclick" => "message()")));
//                    echo $this->Html->link('Promotional news Letter',array('ng-app'=>'#/local_business/news_letter/'.$shop_id.'/' . $model_name. , "javascript:void(0)", array("onclick" => "message()"));
                }
            } else {
                    echo $this->Html->link('Promotional news Letter','#/local_business/news_letter/'. $shop_id . '/' . $model_name, array('ng-href'=>'#/local_business/news_letter/'.$shop_id.'/'.$model_name.'/'));
//                    echo $this->Html->link('Promotional news Letter', array("controller" => "local_business", "action" => "news_letter/" /*. $result["Shop"]["SHOP_CATEGORY"] . "/" */ . $shop_id . '/' . $model_name));
            }?>
