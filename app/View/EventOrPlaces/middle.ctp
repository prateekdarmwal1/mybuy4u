<?php
//debug($results);die;
if (!empty($results['EventPlace'])) {
    $this->assign('head_description', 'mybuy4u - Event/ Place - ' . $results["EventPlace"]["NAME"] . ' - ' . $results["EventPlace"]["DESCRIPTION"]);
    $this->assign('head_keywords', 'mybuy4u - Event/ Place - ' . $results["EventPlace"]["NAME"] . ' - ' . $results["EventPlace"]["DESCRIPTION"]);
}
?>

<div class="yk_template_left_content_tag_changes1">
<div id="row1" class="yk_template_left_content_tag1">
    ...
    ?>
    <?php // if ($name_id[1] == $results['EventPlace']['USER_ID']) { ?>
    <div class="yk_template_left_content_tag1_inner2">
        <div class="yk_template_my_business">
            <div class="ab_left_link_event_place ">
                <span class="yk_product_textq ">
                <?php if ( $this->Session->read('OWNER_NAME_ID_ROLE') AND ($name_id[1] == $results['EventPlace']['USER_ID'])){ ?>
                    <a href="#"
                   class="yk_right_content_link00yk_font_12q rowUp">
                    <?php echo $this->Html->link('My Business', array("controller" => "users", "action" => "edit_profile/" /*. $event_place_id . '/' . $model_name*/));?></a></span>
            <?php } ?>
            </div>
            ...
            <div class="ab_left_link_event_place">
                <span class="yk_product_textq  yk_left_alignvk"><a href="#"
                      class="yk_right_content_link00rowUp yk_font_12q">
                    <?php
                    if ( $this->Session->read('OWNER_NAME_ID_ROLE') AND ($name_id[1] == $results['EventPlace']['USER_ID'])) {
                        echo $this->Html->link('Edit Event', array("controller" => "event_or_places", "action" => "update_business_detail/" . $event_place_id . '/' . $model_name));
                    }else{
                        echo "<a onmousedown = 'jqueryDialog(event)'>Edit Event</a>";
                    }
                    ?></a> </span>
            </div>
            <div class="ab_left_link_event_place"><span class="yk_product_textq  yk_left_alignvk">
                <a href="#"
                   class="yk_right_content_link00rowUp yk_font_12">
                    <?php
                    if($this->Session->read('OWNER_NAME_ID_ROLE') AND ($name_id[1] == $results['EventPlace']['USER_ID'])) {
                        echo $this->Html->link('Adv Setting', array("controller" => "event_or_places", "action" => "adv_setting/" . $event_place_id . '/' . $model_name));
                    }else{
                        echo "<a onmousedown='jqueryDialog(event)'>Adv Setting</a>";
                    }?>
                </a> </span></div>
            <div class="ab_left_link_event_place"><span class="yk_product_textq yk_font_1q2 yk_left_alignvk"><a
                    href="#"
                    class="yk_right_content_link00rowUp yk_font_12q">
                <?php
                if($this->Session->read('OWNER_NAME_ID_ROLE') AND ($name_id[1] == $results['EventPlace']['USER_ID'])) {
                    echo $this->Html->link('Add Product', array("controller" => "event_or_places", "action" => "add_product/" . $event_place_id . '/' . $model_name));
                }else{
                    echo "<a onmousedown='jqueryDialog(event)'>Add Product</a>";
                }?>  </a></span>
            </div>
            <div class="ab_left_link_event_place yk_left_alignvk"><span class="yk_product_textq yk_font_12q"><a
                    href="#"
                    class="yk_right_content_link00rowUp yk_font_12q">
                <?php
                if($this->Session->read('OWNER_NAME_ID_ROLE') AND ($name_id[1] == $results['EventPlace']['USER_ID'])) {
                    echo $this->Html->link('Add Page', array("controller" => "event_or_places", "action" => "add_page/" . $event_place_id . '/' . $model_name));
                }else{
                    echo "<a onmousedown='jqueryDialog(event)'>Add Page</a>";
                }
                ?>
            </a> </span></div>
            <!--<div class="yk_left_link"><span class="yk_product_text yk_font_12"><a href="sales_detail"
                                                                                  class="yk_right_content_link rowUp yk_font_12 yk_left_alignvk">
                Add Sale</a></span></div>-->
        </div>
        <?php
    ...
    <div>
                ...
                <div class="yk_template_publish">
                    <span class="yk_template_publish"><br>
                        <?php if ($this->Session->read("OWNER_NAME_ID_ROLE")) {
                            $name_id = $this->Session->read("OWNER_NAME_ID_ROLE");
                            $name_id = explode(",", $name_id);
                            if ($name_id[1] == $results['EventPlace']['USER_ID']) {
                                echo $this->Html->link('Publisg advertisement in home page', array("controller" => "event_or_places", "action" => "ad_banner/" . $event_place_id . '/' . $model_name));
        ...
        <div class="yk_template_left_content_tag2_inner1 yk_width_tag180">
            <div class="yk_template_slider_content_left_description ">
                ...
                <div class="yk_template_publish yk_top_25 ">
                    <?php echo $this->Html->link('SMS', array("controller" => "event_or_places", "action" => "sms/" . $event_place_id . '/' . $model_name));?>
            ...
            <div class="yk_template_left_content_tag2_inner1 yk_width_tag180">
                <div class="yk_tag2_sales">
                    <span class="yk_template_publish yk_font_12_bold">
                        ...
                        echo $this->Html->link('SALE Promotion', array("controller" => "homes", "action" => "home_sale_detail/"));
                    } ?>
        ...
        <div class=" yk_template_left_content_tag2_inner1  yk_template_left_content_right_div yk_width_tag180 yk_border_none">
            <div class="yk_tag2_sales yk_template_publish yk_font_12_bold">
                ...
                if ($name_id[1] == $results['EventPlace']['USER_ID']) {
                    ?>
                    <?php echo $this->Html->link('Promotional news Letter', array("controller" => "event_or_places", "action" => "news_letter/" /*. $result["Shop"]["SHOP_CATEGORY"] . "/" */ . $event_place_id . '/' . $model_name));?>
                    <?php
                 ...
                 echo $this->Html->link('Promotional news Letter', array("controller" => "event_or_places", "action" => "news_letter/" /*. $result["Shop"]["SHOP_CATEGORY"] . "/" */ . $event_place_id . '/' . $model_name));
            }?>
            </div>
        </div>
    </div>
    ...
    <a href="">
        <div class="yk_template_left_content_tag2_inner1 yk_template_left_content_left_div yk_width_tag180">
            <div class="yk_template_tag2_map">
                                <span class="yk_product_text">
                                    <?php echo $this->Html->link('Map', array("controller" => $this->request->params['controller'], "action" => "Map/" /*. $result["Shop"]["SHOP_CATEGORY"] . "/"*/ . $event_place_id . '/' . $model_name));?>
